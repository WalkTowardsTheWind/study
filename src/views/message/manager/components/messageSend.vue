<template>
  ...
  <send-view-dialog ref="sendViewDialogRef" />
...
import sendViewDialog from "./sendViewDialog.vue";
...
const getList = async () => {
  ...
  loading.value = true;
  try {
    const { data } = await sendIndex(params);
    loading.value = false;
    tableData.length = 0;
...
const handleView = (row: any) => {
  sendViewDialogRef.value.init(row.id);
};
// 公告通知
