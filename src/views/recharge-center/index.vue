<template>
	<zxn-plan>
		<zxn-tabs :tabs-list="tabsList" v-model:activeName="activeName">
			<template #1>
				<div class="recharge">
					累计充值<span class="money">{{ proxy.$moneyFormat(total_amount) }} &nbsp;<span class="text-[14px]">
							元
						</span></span>
				</div>
				<div class="p-[24px]">
					<zxn-search :formItem="formItem" @on-search="handleSearch" @on-reset="handleReset">
						<el-form-item label="">
							<el-input v-model="formItem.name" placeholder="请输入企业名称、税地名称">
								<template #prefix>
									<i-ep-Search />
								</template>
							</el-input>
					...
					<zxn-table :table-data="tableData" :column-list="columnList" hasSelect field-total="amount"
						:page-info="pageInfo" @page-change="pageChange" @selection-change="handleSelect">
						<template #tableTop>
							<el-dropdown class="" trigger="click">
								...
								<template #dropdown>
									<el-dropdown-menu>
										...
										<el-dropdown-item command="2" @click="toUpload">导出</el-dropdown-item>
									</el-dropdown-menu>
								</template>
							</el-dropdown>
						</template>
						<template #certificate="scope">
							<zxn-image :imgList="scope.row.certificate" width="40" height="40" targetClick />
						</template>
						<template #operation="scope">
							<el-button link type="primary" @click="toUpload(scope.row.recharge_id)">导出</el-button>
						</template>
					</zxn-table>
				</div>
			</template>
