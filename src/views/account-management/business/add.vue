<template>
	<zxn-plan>
		<zxn-tabs :tabs-list="tabsList" v-model:activeName="activeName" hasBack></zxn-tabs>
		<zxn-step :step-list="stepList" :active-step="activeStep" />
		<el-form ref="addFormRef" :model="addForm" :rules="rules" class="mt-[52px] ml-[50px]" label-width="125px">
			<el-row :gutter="30" v-show="activeStep === 0">
				<el-col :span="7">
					<el-form-item label="企业登录名称" prop="account" required>
						<el-input placeholder="请输入6位以上英文字母和数字组合" v-model="addForm.account" />
					</el-form-item>
					...
					<el-form-item label="联系方式" prop="mobile" required>
						<el-input maxlength="11" placeholder="请输入手机号或座机号" v-model="addForm.mobile" />
					</el-form-item>
			...
			<el-row :gutter="30" v-show="activeStep === 1">
				<el-col :span="7">
					...
					<el-form-item label="营业执照到期时间">
						<div class="w-full">
							<el-date-picker style="width: 100%" v-model="addForm.license_end_date" unlink-panels
					...
					<el-form-item label="法人身份证号">
						<el-input placeholder="请输入" v-model="addForm.legal_person_idcard" />
					</el-form-item>
					<el-form-item label="法人电话">
						<el-input placeholder="请输入手机号或座机号" v-model="addForm.legal_person_mobile" />
					</el-form-item>
					<el-form-item label="选择行业">
						<el-select class="w-full" placeholder="请选择" v-model="addForm.category_id">
			...
			<el-row :gutter="30" v-show="activeStep === 2">
				<el-col :span="7">
					...
					<el-form-item label="纳税人类型">
						<el-select class="w-full" placeholder="请输入" v-model="addForm.taxpayer_type">
							<el-option v-for="item in taxpayerOptions" :key="item.value" :label="item.label"
			...
			<el-row :gutter="30" v-show="activeStep === 3">
				<el-col :span="8">
					<el-form-item label="企业模式">
						<template #default>
							<div class="busType">
								<div class="radioBtn" :class="addForm.company_source == '0' ? 'is-active' : ''"
									@click="addForm.company_source = '0'">
									销售
								</div>
								<div class="radioBtn" :class="addForm.company_source == '1' ? 'is-active' : ''"
					...
					<template v-if="addForm.company_source == '1'">
						<el-form-item label="上级ID绑定" class="w-full">
							<div style="display: flex; gap: 0 10px">
								...
								<el-input placeholder="请输入" v-model="addForm.parent_channel_id" />
							</div>
						</el-form-item>
					...
					<el-form-item label="企业来源备注">
						<el-input placeholder="请输入" v-model="addForm.company_source_remark" />
					</el-form-item>
					<el-form-item label="点位计费方式" prop="calculation_type">
						<el-select class="w-full" v-model="addForm.calculation_type" placeholder="请选择">
        ...
        <!-- <el-col :span="8">
          <el-form-item>
			...
			<zxn-bottom-btn>
				<el-button type="info" v-if="activeStep === 0" @click="$router.go(-1)">取消</el-button>
				<el-button type="info" v-if="activeStep !== 0" @click="activeStep--">上一步</el-button>
				<el-button type="primary" v-if="activeStep !== 3" @click="activeStep++">下一步</el-button>
				<el-button type="primary" v-if="activeStep === 3" @click="submit(addFormRef)">完成</el-button>
			</zxn-bottom-btn>
		</el-form>
	</zxn-plan>
</template>
