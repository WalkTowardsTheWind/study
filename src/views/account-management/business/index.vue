<template>
	<div class="p-[24px] p-b-[0]">
		<zxn-search :formItem="formItem" @on-reset="handleReset" @on-search="handleSearch">
			<el-form-item>
				<el-input v-model="formItem.company_name" placeholder="请输入企业、账户名称、企业联系人、联系电话">
					<template #prefix>
						<i-ep-Search />
					</template>
				</el-input>
			...
			<el-form-item label="账户状态">
				<el-select v-model="formItem.status" placeholder="请选择" @change="handleChange">
					<el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
				</el-select>
			</el-form-item>
			<el-form-item prop="date" label="创建日期">
				<el-date-picker v-model="date" type="daterange" unlink-panels range-separator="~" start-placeholder="开始日期"
					value-format="YYYY-MM-DD" end-placeholder="结束日期" />
			</el-form-item>
			<el-form-item label="客户点位">
				<div class="flex">
					<el-input v-model="formItem.point_min" placeholder="请输入" clearable />
					<div class="m-x-[20px]">~</div>
					<el-input v-model="formItem.point_max" placeholder="请输入" clearable />
				</div>
			</el-form-item>
          ...
          <template #dropdown>
			...
			<template #operation="scope">
				<el-button v-if="scope.row.status !== 4 &&
					scope.row.status !== 0 &&
					scope.row.status !== 1
					" link type="primary" @click="stop(scope.row.company_id, '2')">封停</el-button>
				<el-button v-if="scope.row.status === 0" link type="primary"
					@click="toDetail('check', scope.row.company_id)">审核</el-button>
				<el-button v-if="scope.row.status === 4" link type="primary"
					@click="free(scope.row.company_id, '1')">解封</el-button>
				<el-button v-if="scope.row.status === 1" link type="primary"
					@click="toDetail('edit', scope.row.company_id)">上传</el-button>
				<el-button link type="primary" @click="toDetail('edit', scope.row.company_id)">编辑</el-button>
				<el-button link type="primary" @click="del(scope.row.company_id)">删除</el-button>
				<!-- <el-button link type="primary">导出</el-button> -->
				<el-button link type="primary" @click="toDetail('detail', scope.row.company_id)">详情</el-button>
