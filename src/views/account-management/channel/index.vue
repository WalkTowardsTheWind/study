<template>
	<div class="p-[24px] p-b-[0]">
		<zxn-search :formItem="formItem" @on-search="handleSearch" @on-reset="handleReset">
			<el-form-item>
			...
			<el-form-item label="账户状态">
				<el-select v-model="formItem.status" placeholder="请选择">
					<el-option v-for="item in statusOptions" :key="item.val" :label="item.label" :value="item.val" />
				</el-select>
			</el-form-item>
			<el-form-item label="渠道类型">
				<el-select v-model="formItem.channel_type" placeholder="请选择">
					<el-option v-for="item in channelOptions" :key="item.val" :label="item.label" :value="item.val" />
				</el-select>
			</el-form-item>
			...
			<el-form-item label="渠道等级">
				<el-select v-model="formItem.level" placeholder="请选择">
					<el-option v-for="item in levelOptions" :key="item.val" :label="item.label" :value="item.val" />
				</el-select>
			</el-form-item>
			<el-form-item prop="date" label="创建日期" label-width="">
				<el-date-picker v-model="date" type="daterange" value-format="YYYY-MM-DD" unlink-panels range-separator="~"
		...
		<zxn-table :table-data="tableData" :column-list="columnList" hasSelect :page-info="pageInfo"
			...
			<template #tableTop>
				<el-dropdown trigger="click">
					...
					<template #dropdown>
						<el-dropdown-menu>
							<el-dropdown-item @click="add('person')">个人渠道</el-dropdown-item>
							<el-dropdown-item @click="add('business')">企业渠道</el-dropdown-item>
							<!-- <el-dropdown-item @click="add(2)">已注册账户</el-dropdown-item> -->
						</el-dropdown-menu>
					</template>
				</el-dropdown>
				<el-button type="primary" plain class="m-l-[15px]">下载</el-button>
          ...
          <template #dropdown>
			...
			<template #operation="scope">
				<el-button v-if="[1, 2, 3].includes(scope.row.status)" link type="primary"
					@click="stop(scope.row.id, 2, scope.row.channel_type)">封停</el-button>
				<el-button v-if="scope.row.status === 4" link type="primary"
					@click="stop(scope.row.id, 1, scope.row.channel_type)">
					解封
				</el-button>
				<el-button v-if="scope.row.status === 0" link type="primary">认证</el-button>
				<el-button v-if="scope.row.status === 1" link type="primary">上传</el-button>
				<el-button link type="primary" @click="toDetail('edit', scope.row)">编辑</el-button>
				<el-button link type="primary" @click="del(scope.row)">删除</el-button>
				<el-button link type="primary" @click="toDetail('detail', scope.row)">详情</el-button>
			</template>
		</zxn-table>
	</div>
</template>
